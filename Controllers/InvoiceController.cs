...
namespace EWC.Controllers
{
    public class InvoiceController : Controller
    {
        ...
        public ActionResult Index(int year = 0)
        {
            ...
            return View(db.Invoices.Include(i => i.customer).Include(i => i.Items).Where(i => i.isActive == true).ToList());
        }
        ...
        {
            var dc = db.deliveryChalans.Include("Items").Include("customer").FirstOrDefault(d=>d.DeliveryChalanID==id);
            ...
            var existinvoice = db.Invoices.Include("Items").Include("customer").FirstOrDefault(d => d.DCNo== dc.DCNo&&d.isActive);
        ...
        public ActionResult Create(Invoice invoice)
        {
            if (ModelState.IsValid)
            {
                ...
                var a = db.deliveryChalans.Where(i => i.DCNo == invoice.DCNo && i.IsActive).FirstOrDefault();
        ...
        public ActionResult Edit(Invoice invoice)
        {
            if (ModelState.IsValid)
            {
                var invindb = db.Invoices.Include(dc => dc.customer).Include(dc => dc.Items).SingleOrDefault(dc => dc.DeliveryChalanID == invoice.DeliveryChalanID && dc.isActive);
        ...
        {
            var invoice = db.Invoices.Include(dc => dc.Items).Include(dc => dc.customer).FirstOrDefault(dc => dc.DeliveryChalanID == id&&dc.isActive);
            if (invoice != null)
            {
